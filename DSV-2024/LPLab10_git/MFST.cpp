	...
	bool Mfst::start(Log::LOG log)
	{
		...
		case NS_NORULE:			MFST_TRACE4("------>NS_NURULE",log)
			...
			*(log.stream) << getDiagnosis(2, buf) << std::endl;
			throw ERROR_THROW_IN(600, num_step);
			break;
		case NS_NORULECHAIN:	MFST_TRACE4("------>NS_NURULENORULECHAIN",log) 
			throw ERROR_THROW_IN(600, num_step);
			break;
		case NS_ERROR:			MFST_TRACE4("------>NS_ERROR",log) 
			throw ERROR_THROW_IN(600, num_step);
			break;
		case SURPRISE:			MFST_TRACE4("------>SURPRISE",log) 
			throw ERROR_THROW_IN(600, num_step);
			break;
		};
		return rc;
