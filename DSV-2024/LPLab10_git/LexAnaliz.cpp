...
namespace Lex
{
	LEX lexAnaliz(Log::LOG log, In::IN in)
	{
		...
		for (int i = 0; word[i] != NULL; i++, indxLex++)
		{
			...
			FST::FST fstShowln(word[i], FST_SHOWLN);
			if (FST::execute(fstShowln))
			{
				LT::Entry entryLT = writeEntry(entryLT, LEX_SHOWLN, LT_TI_NULLIDX, line);
				LT::Add(lextable, entryLT);
				continue;
			}
			...
			if (FST::execute(fstIdentif))
			{
				...
				else
				{
					...
					_mbscpy(tmplvl, lvl); //заходим во временныую область
					word[i] = _mbscat(tmplvl, word[i]);
					idx = IT::IsId(idtable, word[i]); //проверяю наличие id в таблице
					if (idx != TI_NULLIDX)		// если такой идентификатор уже есть
					{
						LT::Entry entryLT = writeEntry(entryLT, LEX_ID, idx, line);
						LT::Add(lextable, entryLT);
